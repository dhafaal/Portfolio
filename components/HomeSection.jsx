...
function HomeSection() {
  ...
  // Scroll-based animations (HOOKS harus di top-level)
  ...
  // ✅ useMemo untuk animasi dekoratif statis
  const bgAnimations = useMemo(
    () => ({
      one: {
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        <motion.div
          animate={bgAnimations.one.animate}
          transition={bgAnimations.one.transition}
          className="absolute top-1/4 -right-20 w-96 h-96 bg-gray-100 rounded-full filter blur-3xl"
        />
        <motion.div
          animate={bgAnimations.two.animate}
          transition={bgAnimations.two.transition}
      ...
      >
        ...
        >
          ...
          >
            A passionate{' '}
            <span className="font-semibold text-black">Frontend Developer</span>{' '}
            who loves building smooth, interactive, and elegant web experiences.
          </motion.p>
          ...
          >
            <SocialLink href="https://github.com/dhafaal" icon="github" />
            <SocialLink
...
// ✅ Komponen kecil + memo
const SocialLink = React.memo(function SocialLink({ href, icon }) {
  const icons = {
    github: (
